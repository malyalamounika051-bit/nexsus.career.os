...
import { Brain, ChevronRight, ChevronLeft, CheckCircle2, Zap } from 'lucide-react';
...
const CareerPredictor = () => {
  ...
  return (
    <div className="app-shell">
      <Sidebar />
      <main className="app-main" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: '2rem', textAlign: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', marginBottom: '0.75rem' }}>
              ...
              <h1 style={{ fontSize: '1.75rem', fontWeight: 800, fontFamily: "'Space Grotesk', sans-serif" }}>
                Career <span className="gradient-text">Predictor</span>
              </h1>
            </div>
            <p style={{ color: 'var(--color-text-muted)', fontSize: '0.9rem' }}>Answer all 8 questions to get your personalized career match</p>
          </motion.div>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', marginBottom: '1.75rem' }}>
                <div style={{ width: 32, height: 32, borderRadius: 8, background: 'var(--color-primary-glow)', border: '1px solid var(--color-border-glow)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: '0.8rem', fontWeight: 700, color: 'var(--color-primary-light)' }}>
                  {current + 1}
                </div>
                <h2 style={{ fontSize: '1.2rem', fontWeight: 700, lineHeight: 1.4, fontFamily: "'Space Grotesk', sans-serif" }}>{q.question}</h2>
              </div>
                  ...
                  return (
                    <motion.button
                      ...
                      style={{
                        ...
                        textAlign: 'left', fontWeight: 500, fontSize: '0.925rem', transition: 'all 0.2s',
                      }}
                    >
                      <span>{opt.label}</span>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            {current < questions.length - 1 ? (
              ...
              <button
                ...
                style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', opacity: loading || !answers[q.id] ? 0.7 : 1 }}
              >
                {loading ? 'Analyzing...' : (<><Zap size={18} /> Get Results</>)}
              </button>
            )}
          </div>
        </div>
